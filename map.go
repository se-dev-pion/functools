...
func Map4Slice[T, U any, E ~[]T, R ~[]U](handler func(T) U, entry E) R {
...
func Map4String(handler func(string) string, entry string) string {
	output := make([]rune, 0)
...
func Map4Chan[T, U any, E ~chan T, R ~chan U](handler func(T) U, entry E) R {
	output := make(R, len(entry))
